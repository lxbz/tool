
#include "stdafx.h"
#include "encrypttest.h"

//#include "StringHash.h"

#define CRYPT_TYPE_USER_INDEX	4		///< 用户注册类型最小索引
#define CRYPT_TYPE_USER_MASK	0xF0	///< 用户注册类型MASK

/*
void generateRandXorList()
{
TCHAR buf[256];

::srand(::time(0));

for (int i=0; i<256; i++)
buf[i] = i;

for (int i=0; i<256; i++)
{
int idx = rand() & 0xff;
TCHAR tmp = buf[idx];
buf[idx] = buf[i];
buf[i] = tmp;
}

char str[256];

// 编码缓冲
for (int i=0; i<256; i++)
{
if ((i & 15) == 0)
::OutputDebugString("\r\n");

sprintf(str, "0x%02X, ", buf[i]);
::OutputDebugString(str);
}

// 解码缓冲
for (int i=0; i<256; i++)
{
for (int j=0; j<256; j++)
{
if (buf[j] == i)
{
if ((i & 15) == 0)
::OutputDebugString("\r\n");
sprintf(str, "0x%02X, ", j);
::OutputDebugString(str);
}
}
}
return true;
}*/
bool encode_map(BYTE* buf, size_t len, DWORD /*key*/)
{
	static const BYTE encode_buf[256] = {
		, 
	};
	for (size_t i=0; i<len; i++)
		buf[i] = encode_buf[buf[i]];
	return true;
}

bool decode_map(BYTE* buf, size_t len, DWORD /*key*/)
{
	static const BYTE decode_buf[256] = {
		, 
	};
	for (size_t i=0; i<len; i++)
		buf[i] = decode_buf[buf[i]];
	return true;
}

/** 加（解）密算法表*/
typedef struct tagCryptFunsTable
{
	DWORD			mask;				///< 加（解）密算法mask
	CRYPT_FUNS		encrypt;			///< 加密函数指针
	CRYPT_FUNS		decrypt;			///< 解密函数指针
} CryptFunsTable, *PCryptFunsTable;


CryptFunsTable g_cryptFuncTable[MAX_CRYPT_TYPE_COUNT] =
{
	{ CRYPT_TYPE_EMPTY, (CRYPT_FUNS)0,	(CRYPT_FUNS)0 },
	{ CRYPT_TYPE_1,		(CRYPT_FUNS)encode_map,	(CRYPT_FUNS)decode_map },
	//{ CRYPT_TYPE_2,		makeMap,	makeMap },
};

bool encrypt(TCHAR* buffer, DWORD size, DWORD key, DWORD mask)
{
	//PROFILE("Encrypt");
	if (buffer == NULL || size == 0)
		return false;

	if (mask == 0) // ingore!
		return true;

	bool validMask = false;
	for (int i = 1; i < MAX_CRYPT_TYPE_COUNT; i++)
	{
		if (mask & g_cryptFuncTable[i].mask)
		{
			validMask = true;
			CRYPT_FUNS encryptFuns = g_cryptFuncTable[i].encrypt;
			if (encryptFuns && !((*encryptFuns) (buffer, size, key)))
				return false;
		}
	}

	return validMask;
}

bool decrypt(TCHAR* buffer, DWORD size, DWORD key, DWORD mask)
{
	//PROFILE("Decrypt");
	if (buffer == NULL || size == 0)
		return false;

	if (mask == 0) // ingore!
		return true;

	bool validMask = false;
	for (int i = 1; i < MAX_CRYPT_TYPE_COUNT; i++)
	{
		if (mask & g_cryptFuncTable[i].mask)
		{
			validMask = true;
			CRYPT_FUNS decryptFuns = g_cryptFuncTable[i].decrypt;
			if (decryptFuns && !((*decryptFuns) (buffer, size, key)))
				return false;
		}
	}

	return validMask;
}

DWORD registerCrypt(CRYPT_FUNS encryptFuns, CRYPT_FUNS decryptFuns)
{
	if (encryptFuns == NULL || decryptFuns == NULL)
		return 0;

	for (int i = CRYPT_TYPE_USER_INDEX; i < MAX_CRYPT_TYPE_COUNT; i++)
	{
		if (g_cryptFuncTable[i].encrypt == NULL && g_cryptFuncTable[i].decrypt == NULL)
		{
			g_cryptFuncTable[i].mask = 1 << i;
			g_cryptFuncTable[i].encrypt = encryptFuns;
			g_cryptFuncTable[i].decrypt = decryptFuns;
			return g_cryptFuncTable[i].mask;
		}
	}

	return -1;
}

bool unregisterCrypt(DWORD mask)
{
	mask &= CRYPT_TYPE_USER_MASK;
	if (mask == 0)
		return false;

	for (int i = CRYPT_TYPE_USER_INDEX; i < MAX_CRYPT_TYPE_COUNT; i++)
	{
		if (mask & g_cryptFuncTable[i].mask)
		{
			g_cryptFuncTable[i].mask = 0;
			g_cryptFuncTable[i].encrypt = NULL;
			g_cryptFuncTable[i].decrypt = NULL;
		}
	}

	return true;
}

int getCryptCount()
{
	int count = 1; // index = 0, no crypt type
	for (int i = 1; i < MAX_CRYPT_TYPE_COUNT; i++)
	{
		if (g_cryptFuncTable[i].encrypt != NULL || g_cryptFuncTable[i].decrypt != NULL)
			count ++;
	}
	return count;
}