...
/**  加（解）密函数原型*/
typedef bool (*CRYPT_FUNS) (TCHAR* buffer, DWORD size, DWORD key);
...
*/
bool encrypt(TCHAR* buffer, DWORD size, DWORD key, DWORD mask);
...
*/
bool decrypt(TCHAR* buffer, DWORD size, DWORD key, DWORD mask);
...
@see unregisterCrypt
*/
DWORD registerCrypt(CRYPT_FUNS encryptFuns, CRYPT_FUNS decryptFuns);
...
@see registerCrypt
*/
bool unregisterCrypt(DWORD mask);
